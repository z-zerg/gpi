...
$Destination="D:\123" #папка назначения
$Versions="50" #Количество резервных копий
$BackupDirs=
"D:\keys",
"D:\PS_scripts", 
"D:\1Cbase\БитФинанс(рабочая).dt", 
"C:\MonitoringAuto" #
$Log="Log.txt" #Файл журнала
...
$Backupdir=$Destination +"\Backup-"+ (Get-Date -format yyyy.MM.dd_H-mm-ss)+"\"
$Items=0
$Count=0
$ErrorCount=0
...
function Check-Dir {
    ...
    if (!(Test-Path $BackupDirs)) {
        return $false
        Logging "Error" "Путь $BackupDirs недоступна"
    }
    if (!(Test-Path $Destination)) {
        return $false
        Logging "Error" "Путь $Destination недоступен"
    }
}
